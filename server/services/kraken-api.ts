...
export class KrakenApiService {
  ...
  private generateAuthHeaders(
    ...
    const message =
      endpoint + crypto.createHash("sha256").update(postData).digest();
  ...
  private async makePrivateRequest<T>(
    ...
    try {
      ...
      console.error(
        `Kraken private API request failed for ${endpoint}:`,
        error
      );
      throw error;
    }
  ...
  async getTickerData(pair: string): Promise<TickerData> {
    ...
    return {
      ...
      changePercent24h:
        ((parseFloat(data.c[0]) - parseFloat(data.o)) / parseFloat(data.o)) *
        100,
      ...
      spreadPercent:
        ((parseFloat(data.a[0]) - parseFloat(data.b[0])) /
          parseFloat(data.b[0])) *
        100,
  ...
  async getOrderBook(pair: string, count: number = 5): Promise<OrderBook> {
    const result = await this.makeRequest<any>(
      `/public/Depth?pair=${pair}&count=${count}`
    );
    const pairKey = Object.keys(result)[0];
    const data = result[pairKey];
  ...
  async getRecentTrades(
    ...
    const result = await this.makeRequest<any>(
      `/public/Trades?pair=${pair}&count=${count}`
    );
    const pairKey = Object.keys(result)[0];
    const trades = result[pairKey];

    const processedTrades = trades.map(
      (trade: [string, string, number, string, string, string]) => ({
  ...
  async getMultipleTickers(pairs: string[]): Promise<MarketData[]> {
    ...
    return pairs.map((pair) => {
      ...
      if (!pairKey || !result[pairKey]) {
        console.warn(
          `No data found for pair ${pair}. Available keys:`,
          Object.keys(result)
        );
